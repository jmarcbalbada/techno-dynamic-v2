...
const GeneralContextList = ({ lessonId }) => {
  ...
  return (
    <Box>
      {generalContexts.map((context, index) => (
        <Accordion
          key={context.related_content__related_content_id}
          onChange={() =>
            setSelectedGeneralContext(
              context.related_content__related_content_id
            )
          }
          sx={{
          ...
          <AccordionSummary expandIcon={<ExpandMoreIcon />}>
            <Typography
              ...
              }>{`${context.related_content__general_context} (${context.count})`}</Typography>
          </AccordionSummary>
          <AccordionDetails>
            {selectedGeneralContext ===
              context.related_content__related_content_id && (
              <QuestionList
                lessonId={lessonId}
